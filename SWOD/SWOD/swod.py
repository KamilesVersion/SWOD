...
def artist_top_tracks():
    ...
    top_tracks = []
    total_minutes_listened = 0  # <-- naujas kintamasis
    sp = spotify.get_spotify_client()
    ...
    if artist_name:
        results = (
            db.session.query(
                ...
                ListeningHistory.artist_name,
                db.func.max(ListeningHistory.duration_ms).label('duration_ms')  # <-- pridėta trukmė
            )
            .filter_by(user_id=current_user.id)
        ...
        for track in results:
            ...
            total_minutes_listened += (duration_ms * track.listen_count) / 60000  # konvertuojam į minutes
...
def generate_playlist():
    ...
    # Spotify user ID (not from DB, but from actual Spotify profile)
    spotify_user = sp.current_user()
    spotify_user_id = spotify_user['id']

    # Flask-Login user ID for your DB
    db_user_id = current_user.id

    # Initialize playlist name and track query
    track_query = None
    playlist_name = None

    if playlist_type == 'top20':
        playlist_name = "SWOD - Top 20 Songs"
        track_query = (
            db.session.query(
                ListeningHistory.track_name,
                ListeningHistory.artist_name,
            ...
            .filter(ListeningHistory.user_id == db_user_id)
            .group_by(ListeningHistory.track_name, ListeningHistory.artist_name)
            .order_by(db.desc('listen_count'))
            .limit(20)
            .all()
        )

    elif playlist_type == 'recent20':
        playlist_name = "SWOD - Recent 20 Songs"
        track_query = (
            db.session.query(ListeningHistory.track_name, ListeningHistory.artist_name)
            .filter(ListeningHistory.user_id == db_user_id)
    ...
    elif playlist_type == 'weekly_top20':
        playlist_name = "SWOD - Top 20 This Week"
        ...
        track_query = (
            ...
            .filter(ListeningHistory.user_id == db_user_id)
            .filter(ListeningHistory.played_at >= one_week_ago)
            .group_by(ListeningHistory.track_name, ListeningHistory.artist_name)
            .order_by(db.desc('listen_count'))
            .limit(20)
            .all()
        ...
        return "Invalid playlist type", 400
    ...
    if not track_uris:
        flash("No matching tracks found on Spotify.")
        return redirect(url_for('menu'))

    # Create and populate the playlist
    playlist = sp.user_playlist_create(user=spotify_user_id, name=playlist_name, public=False, description=f"{playlist_name} generated by SWOD.")
    sp.playlist_add_items(playlist_id=playlist['id'], items=track_uris)

    return render_template("playlist_link.html", playlist_url=playlist['external_urls']['spotify'])
